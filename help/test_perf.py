...
##import rank_based
from PER import PER_proportional
...
##    per4 = rank_based.Experience({'size': 1000,'learn_start': 10,'partition_num': 4,'total_step': 100,'batch_size': 4})
	per_prop = PER_proportional(size=1000, alpha=0.6, beta_zero=0.4, batch_size=4, annealing_beta_steps=15)
	for i in range(1000):
...
##        per4.store((np.ones((84,84)), 1, 1, np.ones((84,84)), True))
		per_prop.add(np.random.rand(1)[0], (np.ones((84,84)), 1, 1, np.ones((84,84)), True))
...
##    print("Execution time: ", tf4)
	...
	t = time.time()
	for i in range(5000):
		a,b,c = per_prop.sample(4)
	print(c)
	tf5 = time.time() - t
	print("Execution time: ", tf5)
