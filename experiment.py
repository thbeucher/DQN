...
from PER import PER_proportional as PER
...
def run_experiment():
	...
	with tf.Session() as sess:
		...
		if LOAD_MODEL == False or D_loaded == False:
			...
			#for i in range(REPLAY_MEMORY_SIZE/5): # without PER
			for i in range(REPLAY_MEMORY_SIZE): # with PER
				...
				if PER_PROP:
					#if we use PER proportional version, we initialize
					#priority of the transition with the reward
					r = 1 if r < 0 else r
					D.add(r, (state, action_index, r, s1, t))
				else:
		...
		while 1:
			...
			if PER_PROP:
				terminal_or_not_multiplier = -(t - 1)
				Q_value = targetDQN.output_layer.eval(feed_dict={targetDQN.network_input:[s1]})
				Q_batch = np.max(Q_value, 1)[0]
				p = r + (GAMMA * Q_batch * terminal_or_not_multiplier)
				p = 1 if p < 0 else p
				D.add(p, (state, a, r, s1, t))
			else:
